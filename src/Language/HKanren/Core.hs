...
unifyTerms
  :: (HFoldable h, Unifiable h h, HOrdHet (Sing h))
...
occursCheck
  :: forall h ix. (HFoldable h, HOrdHet (Sing h))
...
-- normalization.
canonize :: forall h ix. (HFunctorId h, HOrdHet (Sing h)) => Subst h -> Term h ix -> Term h ix
...
checkNeqs :: forall h. (HFunctorId h, HFoldable h, Unifiable h h, HOrdHet (Sing h)) => State h -> Logic (State h)
...
-- this is impossible.
(===) :: (HFunctorId h, HFoldable h, Unifiable h h, HOrdHet (Sing h))
...
(===*) :: (HFunctorId h, HFoldable h, Unifiable h h, HOrdHet (Sing h), HEqHet (h (Term h)))
       => Term h ix -> Term h ix' -> Predicate h
(===*) l r =
  case heqIx l r of
    Just Refl -> l === r
...
fresh :: (HFoldable h, Unifiable h h, SingI h ix)
...
run :: forall h ix. (HFunctorId h, HFoldable h, Unifiable h h, SingI h ix, HOrdHet (Sing h))
