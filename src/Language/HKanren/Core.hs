...
unifyTerms
  :: (HFoldable h, Unifiable h h, HOrdHet Sing)
...
occursCheck
  :: forall h ix. (HFoldable h, HOrdHet Sing)
...
-- normalization.
canonize :: forall h ix. (HFunctorId h) => Subst h -> Term h ix -> Term h ix
...
checkNeqs :: forall h. (HFunctorId h, HFoldable h, Unifiable h h, HOrdHet Sing) => State h -> Logic (State h)
...
-- this is impossible.
(===) :: (HFunctorId h, HFoldable h, Unifiable h h, HOrdHet Sing)
...
(===*) :: (HFunctorId h, HFoldable h, Unifiable h h, HOrdHet Sing, HEqHet (h (Term h)))
       => Term h ix -> Term h ix' -> Predicate h
(===*) l r =
  case heqIx l r of
    Just Refl -> l === r
...
fresh :: (HFoldable h, Unifiable h h, SingI ix)
...
run :: forall h ix. (HFunctorId h, HFoldable h, Unifiable h h, SingI ix)
