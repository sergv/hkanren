...
data AtomF (f :: * -> *) ix where
  Atom :: String -> AtomF f Atom
...
instance (HFoldable h, HOrdIx (h HUnit), Unifiable h h, PairF :<: h) => Unifiable PairF h where
...
instance (HEqIx f) => HEqIx (PairF f) where
...
instance (HOrdIx f) => HOrdIx (PairF f) where
...
type LispTermF = PairF :+: AtomF
type LispTerm = Term LispTermF

templateAtom :: (AtomF :<: h) => h f Atom
templateAtom = inj $ Atom "test"

templatePair :: (PairF :<: h) => h HUnit (Pair () ())
templatePair = inj $ Pair (K ()) (K ())
...
hasSolution :: (HFunctor h, HFoldable h, Unifiable h h, HOrdIx (h HUnit), AtomF :<: h)
            => Predicate h -> Bool
hasSolution p =
...
list :: [Some (LispTermF LispTerm)] -> Some (LispTermF LispTerm)
list = foldr (\(Some x) (Some y) -> Some $ inj $ Pair (HFree x) (HFree y)) (Some $ inj $ Atom "nil")
